...
def delete_rareWords(texts):
    """
    去除 在整个语料库中仅出现一次的单词
    :param texts: 原始语料库 [[每篇window中的具体的词，用列表存储],[],[]...]
    :return:
    """
...
class InitialCorporaUtil(object):
    ...
    def __init__(self):
        ...
        # _texts: [[每篇window中的具体的词，用列表存储],[],[]...]
        self._texts = []

        # # 字典
        # self._dictionary = {}

        # _list_corpora = [[(),(),()],[],[]....]   最终状态（编号，计数）
        self._list_corpora = []




    def readWindowTxt(self, dest):
        """
        ...
        for root, dirs, files in os.walk(dest):
            for OneFileName in files:
                ...
                if len(windowCounter) == 0:  # 如果这个window.txt没有找到words就continue
                    continue
        ...
        self._texts = delete_rareWords(self._texts)
    ...
    def savetoDisk(self, dictionaryLocation, corporaLocation):
        ...
        :return:  保存结果到文件
        """
        if not os.path.isdir(GLOBAL_generatedFiles):
